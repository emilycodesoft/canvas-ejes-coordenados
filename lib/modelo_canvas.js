...
class EjesCoordenados {
  ...
  /* dibujarNumeros(eje, ejeSecundario, lado, sentido, sentidoD) {
    let iEje = 0;
    let lineasEje = [];
    let ejeInicial = eje;
    //let ejeSecundadario = // if operator
    while (ejeInicial < eje + lado) {
      this.mover(ejeInicial, ejeSecundario + 10); // el espacio entre cada numero va a ser de 10
      lineasEje.push(new Linea(ejeInicial, ejeSecundario - 10));
      if (sentido == 0) {
        new Texto(
          (ejeInicial - eje).toString(),
          ejeInicial - 20,
          ejeSecundario - 40,
          `${this.tamañoTexto}px Arial`
        ).dibujar();
      } else {
        new Texto(iEje.toString(), ejeInicial - 20, ejeSecundario - 40, `${this.tamañoTexto}px Arial`).dibujar();
        iEje += sentido;
      }
      ejeInicial += sentidoD;
    }
  ...
  dibujarNumerosY() {
    ...
    while (yInicio < this.y + this.largo) {
      ...
      lineasEjeX.push(new Linea(this.x - 10, yInicio));
      if (this.sy == 0) {
  ...
  dibujarNumerosX() {
    ...
    while (xInicio < this.x + this.ancho) {
      ...
      lineasEjeX.push(new Linea(xInicio, this.y - 10));
      if (this.sx == 0) {
